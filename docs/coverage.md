...
3. If configured, keeps only targets generated by allowed top-level BUILD rule
   calls.
...
### Limiting coverage by BUILD definition

Large repositories can use `please.coverage.allowedBuildDefs` to prevent the
extension from running tests created by unrelated BUILD definitions:
...
The values are top-level function names as written in a BUILD file, not target
labels. Given this call:

```python
variant_go_test(
    name = "calculator_sat_test",
    variants = ["addition", "subtraction"],
)
```

the extension associates generated targets such as
`:calculator_sat_test_addition` and `:calculator_sat_test_subtraction` with
`variant_go_test` through their shared `calculator_sat_test` name prefix.

An empty list, which is the default, allows every directly dependent test
target. The allowlist filters the result of the direct reverse-dependency
query; it reduces the metadata queried and the targets passed to `plz cover`,
but it does not remove the initial `plz query revdeps` operation.
...
Set `please.runInTerminal` to choose where the coverage command runs:
...
When enabled, `plz cover` runs in the integrated terminal and retains Please's
coloured terminal output. When disabled, the default, the extension opens the
**Please** Output channel before target discovery and streams the command there.
Discovery errors always open the Output channel so they remain visible.
...
- BUILD-definition filtering still performs the initial direct
  reverse-dependency query before applying the configured allowlist.
...
- For a source file, confirm a test target depends directly on its source
  target.
- If `please.coverage.allowedBuildDefs` is set, confirm it contains the
  top-level BUILD function name rather than a generated target label.
